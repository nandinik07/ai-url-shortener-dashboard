...
export interface DbService {
  ...
  close(): Promise<void>;
  createLink(title: string, originalUrl: string, shortCode: string, customAlias: string | null, expiresAt: Date | null): Promise<Link>;
  getLinkById(id: number): Promise<Link | null>;
  getLinkByCode(shortCode: string): Promise<Link | null>;
  getLinkByAlias(alias: string): Promise<Link | null>;
  listLinks(options: { limit: number; offset: number; search?: string }): Promise<{ links: LinkStats[]; total: number }>;
  updateLink(id: number, data: { title?: string; original_url?: string; custom_alias?: string | null; expires_at?: Date | null; is_active?: boolean }): Promise<Link>;
  softDeleteLink(id: number): Promise<boolean>;
  recordClick(linkId: number, browser: string, os: string, country: string, referrer: string): Promise<void>;
  getLinkAnalytics(linkId: number): Promise<{
...
class PostgresDbService implements DbService {
  ...
  private parseLink(row: any): Link {
    return {
      id: row.id,
      title: row.title,
  ...
  async createLink(title: string, originalUrl: string, shortCode: string, customAlias: string | null, expiresAt: Date | null): Promise<Link> {
    const query = `
  ...
  async getLinkByCode(shortCode: string): Promise<Link | null> {
  ...
  async listLinks(options: { limit: number; offset: number; search?: string }): Promise<{ links: LinkStats[]; total: number }> {
    ...
    const links = res.rows.map((row: any) => ({
      ...this.parseLink(row),
  ...
  async updateLink(id: number, data: { title?: string; original_url?: string; custom_alias?: string | null; expires_at?: Date | null; is_active?: boolean }): Promise<Link> {
    const fields: string[] = [];
    const params: any[] = [];
    let paramIndex = 1;
  ...
  async softDeleteLink(id: number): Promise<boolean> {
    const res = await this.pool.query(
      "UPDATE links SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL RETURNING id",
      [id]
    );
    return res.rows.length > 0;
  }

  async recordClick(linkId: number, browser: string, os: string, country: string, referrer: string): Promise<void> {
    const query = `
  ...
  async getLinkAnalytics(linkId: number): Promise<any> {
    const dateQuery = `
...
class SqliteDbService implements DbService {
  ...
  async createLink(title: string, originalUrl: string, shortCode: string, customAlias: string | null, expiresAt: Date | null): Promise<Link> {
    ...
    const res = await this.db.run(
      "INSERT INTO links (title, original_url, short_code, custom_alias, expires_at) VALUES (?, ?, ?, ?, ?)",
      [title, originalUrl, shortCode, customAlias, expiresAtStr]
    );
...
if (process.env.USE_SQLITE === "true" || process.env.NODE_ENV === "test") {
  ...
  db.initialize = async (): Promise<void> => {
    try {
      await originalInit();
