...
class RedirectController {
    ...
    static async redirect(req, res) {
        ...
        try {
            ...
            let targetUrl = await cache_1.cache.get(shortCode);
            let linkId = null;
            if (!targetUrl) {
                // 2. Cache Miss: Query Database (Postgres/SQLite)
                // Check both shortCode and custom_alias
                let link = await db_1.db.getLinkByCode(shortCode);
                if (!link) {
                    link = await db_1.db.getLinkByAlias(shortCode);
                }
          ...
          `);
                ...
                await cache_1.cache.set(shortCode, targetUrl, 3600);
            }
            ...
            (async () => {
                try {
                    ...
                    if (linkId) {
                        const metrics = (0, uaParser_1.parseUserAgent)(userAgent);
                        const country = await (0, geo_1.getCountryFromIp)(clientIp);
                        await db_1.db.recordClick(linkId, metrics.browser, metrics.os, country, referrer);
                    }
                }
